...
export default function AgeGate() {
  ...
  const handleEnter = async () => {
    ...
    // Fire-and-forget email capture — non-blocking, never prevents entry
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="relative w-full max-w-md bg-parchment brass-double-border p-8 md:p-10 shadow-2xl">
        ...
        {/* Optional email — only shown when age is checked */}
        {checked && (
          <div className="mb-5 transition-all">
            <p className="font-display text-[10px] tracking-[0.15em] uppercase text-espresso mb-2">
              Get early access alerts <span className="font-body normal-case tracking-normal text-saddle/60">(optional)</span>
            </p>
            <input
              type="email"
              value={email}
